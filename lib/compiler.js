...
module.exports = Compiler = (function() {
    ...
    if (atomSassActivated === false) {
      ...
      atom.project.observeBuffers(function(buffer) {
        ...
        textBuffer.onDidSave(function(e) {
          ...
          || (!atom.config.get("atom-sass.compilePartials") && path.basename(saveFilePath).startsWith("_"))) {
            return false
          }
  ...
  Compiler.prototype.compile = function(sassFilePath) {
    ...
    exec(`sass ${atom.config.get("atom-sass.additionalParameters")} "${sassFilePath}" "${cssFilePath}"`, function (error, stdout, stderr) {
      if (error) {
